...
class cartController {
    ...
    static findOne(req, res) {
        ...
        Cart
            ...
            .then((findOneCart) => { 
                res.status(200).json(findOneCart) 
            })
            .catch((err) => { res.status(500).json(err) })
    }
    ...
    static removeCart(req,res){
        ...
        Cart
            .findOne({ UserId: id})
            .then(findOneCart =>{
                findOneCart.products.forEach(element => {
                    if(String(element.ProductId) !== ProductId){
                        cart.push(element)
                    }
                });
                return Cart.updateOne({ UserId:id }, { $set: { products: cart }})
            })
            .then((updatedCart) => {
                res.status(200).json({ message: 'Success Deleting this product on cart' , updatedCart})
            })
            .catch(err => {
                console.log(err)
            })
    ...
    static removeCartUser(req,res){
        const { id } = req.authenticatedUser
        Cart
            .updateOne({ UserId: id},{ $set: { products: [] }})
            .then((updatedCart) => {
                res.status(200).json({ message: 'Success to checkout' , updatedCart})
            })
            .catch(err => {
                console.log(err)
            })
