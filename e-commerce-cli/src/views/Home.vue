<template>
  <div>
    ...
    <b-container class="d-flex listProduct">
      <b-row v-if="productList" class="listProduct" align-h="between">
        <b-col sm="3" lg="2" class="itemBox" v-for="(list,index) in productList.data" :key="index">
          <b-row>
            <h3 id="productName" style="margin-top: 5px; text-align:center; color:#790617">
          ...
          <b-row>
            <b-col style="height:180px;"><img :src="list.img" id="imgProduct"
                ...
                style="font-size:25px; right:22%; margin-top:20px; color: red; position:absolute ;font-weight: 900">
                <strong>OUT OF STOCK</strong> </span></b-col>
          </b-row>
          <b-row style="margin-bottom:-20px">
            <h5 id="productPrice" ><strong>Rp {{list.price.toLocaleString()}}</strong></h5>
          </b-row>
          <b-row>
            ...
            <span v-if="list.stock <= 0" style="margin-top:10px; color: red; font-weight: 900">
...
<style>
  ...
  .itemBox {
    ...
    height: auto;
    margin: 20px;
    background-color: rgba(255, 218, 218, 0.274)
  }
